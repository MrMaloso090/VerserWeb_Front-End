// DESPLEGAR LAS OPCIONES QUE HAY DESPUES DE GARANTIA //
const tipoIngreso = document.getElementById('tipo_ingreso');  // Toma el <select> donde eliges el tipo de ingreso
const camposGarantia = document.getElementById('campos_garantia'); // Contenedor con los campos extra (motivo_garantia, orden_origen)

// Escucha cuando cambie la opción seleccionada
tipoIngreso.addEventListener('change', () => {
  const motivo = document.getElementById('motivo_garantia');
  ...
  if (tipoIngreso.value !== "" && tipoIngreso.value !== "NORMAL" && tipoIngreso.value !== "NORMAL AGENCIA CALI" && tipoIngreso.value !== "NORMAL AGENCIA CUCUTA") {
    ...
    if (tipoIngreso.value == "ACOMPAÑAMIENTO COMERCIAL") {
      const AComercial = ["CAMBIO DE FORMULA", "CAMBIO DE DISEÑO (SEGMENTO)", "CAMBIO DE MEDIDAS (DP, ALTURA, CORREDOR, ETC)", "CAMBIO DE MATERIAL", "CAMBIO DE MONTURA", "ADAPTACION"]
      motivo.replaceChildren(new Option("", "")); // limpia
      AComercial.forEach(opt => {
      ...
      const otro = ["MARQUILLAS", "MAL SUMINISTRADO", "RAYA-TALLON", "FORMULA", "DAÑO EN LA MONTURA", "MATERIAL", "MAL CALCULADO", "MAL AR-MAL COATING"]
      motivo.replaceChildren(new Option("", "")); // limpia
      otro.forEach(opt => {
        const option = document.createElement("option");
          option.value = opt;
    ...
    motivo.value = "";
    orden.value = "";
    // oculta el bloke si no se cumplesn las condiciones
    camposGarantia.style.display = "none";  // Oculta los campos si no aplica
    document.getElementById('motivo_garantia').required = false;  // Desactiva el "required"
    document.getElementById('orden_origen').required = false;     // Desactiva el "required"
  }
});
////
 
// FUNCION PARA LA IMPORTACION DE LA LISTA DE CLIENTES DE MYSQL A AL SELECT DE LA WEB //
...
// Ejecutar la funcion al abrir la página
cargarClientes();
////
...
window.onload = () => {
  const sesion = sessionStorage.getItem("usuario_ingresos");
...
async function verificacion() {
  ...
  const pag = "ingresos"; //NOMBRE DE LA PAGINA, PARA SU IDENTIFICACION
  ...
  try {
    ...
    const respuesta = await resp.json(); //RESPUESTA DEL BACK-END

    if (respuesta.valido) { // RESPUESTA DEL BACK-END INFORMANDO SI VALIDO ES TRHUE O FALSE
      sessionStorage.setItem("usuario_ingresos", respuesta.nombre); // RESPUESTA DEL BACK-END CON EL NUMBRE DE USUARIO
      mostrarContenido(respuesta.nombre); // EJECUTA LA FUNCION QUE MUESTRA EL CONTENIDO Y COMPARTE EL NOMBRE DE USUARIO
    } else {
...
function cerrarSesion() {
  sessionStorage.removeItem("usuario_ingresos");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const datos = {
      ...
      numero_de_gaveta: form.numero_gaveta.value.trim(),
      ...
      condicion_especial: form.condicion_especial.value,
      
      usuario: sessionStorage.getItem("usuario_ingresos") // SE ESPESIFICA EN CUAL WEB ESTAMOS //
    };
    ...
    try {
      ...
      if (respuesta.complete) {
        ...
        // oculta el bloke que contiene motvio y numero de garantia, cuando se envia el formulario. (por si acaso es necesario)
        camposGarantia.style.display = "none";  // Oculta el bloke
        document.getElementById('motivo_garantia').required = false;  // Desactiva el "required"
        document.getElementById('orden_origen').required = false;     // Desactiva el "required"
        return;
      }
